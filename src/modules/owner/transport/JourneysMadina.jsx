...
const JourneysMadina = () => {
  ...
  const loadData = async () => {
    ...
    const [runRes, driverRes, vehicleRes, resRes] = await Promise.all([
      supabase.from('transport_runs').select('*').eq('journey_type', 'madina').order('planned_start', { ascending: false }),
  ...
  const openDrawer = (run = null) => {
    if (run) {
      ...
      const restStops = notesObj && notesObj.rest_stops ? notesObj.rest_stops.join('\n') : '';
      const secondaryDriverId = notesObj && notesObj.secondary_driver_id ? notesObj.secondary_driver_id : '';

      setForm({
        reservation_id: run.reservation_id || '',
        planned_start: run.planned_start ? run.planned_start.substring(0, 16) : '',
        planned_end: run.planned_end ? run.planned_end.substring(0, 16) : '',
  ...
  const validate = () => {
    ...
    if (form.planned_start && form.planned_end) {
      const start = new Date(form.planned_start);
      const end = new Date(form.planned_end);
      const diffHours = (end - start) / (1000 * 60 * 60);

      if (diffHours > 4 && !form.secondary_driver_id) {
        errors._warning = `Trip duration is ${diffHours.toFixed(1)}h (>4h). Secondary driver recommended for fatigue management.`;
      }
    }
  ...
  const handleSave = async () => {
    ...
    const hasWarning = errors._warning;
    delete errors._warning;
    ...
    if (hasWarning) {
      if (!confirm(`${hasWarning}\n\nContinue without secondary driver?`)) {
        return;
      }
    }
    ...
    const restStopsArray = form.rest_stops.trim() ? form.rest_stops.split('\n').map(s => s.trim()).filter(Boolean) : [];
    const notesObj = {
      rest_stops: restStopsArray,
      secondary_driver_id: form.secondary_driver_id || null
    };
    ...
    if (editing) {
      ...
      setToast({ type: 'success', message: 'Madina journey updated' });
    } else {
      const { error: err } = await supabase
        .from('transport_runs')
        .insert([payload]);
      ...
      setToast({ type: 'success', message: 'Madina journey created' });
    }
  ...
  const handleDelete = async (id) => {
    if (!confirm('Delete this Madina journey?')) return;
    const { error: err } = await supabase
      .from('transport_runs')
      .delete()
      .eq('id', id);
    ...
    if (err) {
      ...
      setToast({ type: 'success', message: 'Madina journey deleted' });
      loadData();
    }
  };
  ...
  return (
    <div className="p-4 sm:p-6 pb-24">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold">Madina Journeys</h1>
        <button
          onClick={() => openDrawer()}
      ...
      <div className="space-y-3">
        {runs.map(run => {
          ...
          const notesObj = safeJson(run.notes);
          const restStops = notesObj && notesObj.rest_stops ? notesObj.rest_stops : [];
          const secondaryDriverId = notesObj && notesObj.secondary_driver_id ? notesObj.secondary_driver_id : null;
          const secondaryDriver = secondaryDriverId ? drivers.find(d => d.id === secondaryDriverId) : null;
          const duration = getDuration(run.planned_start, run.planned_end);
          ...
          return (
            <div key={run.id} className="bg-white border rounded-lg p-4 shadow-sm">
              ...
              <div className="text-sm space-y-1 mb-2 text-gray-600">
                <div>Primary Driver: {driver ? driver.name : 'Missing/Invalid data'}</div>
                {secondaryDriver && (
                  <div>Secondary Driver: {secondaryDriver.name}</div>
                )}
                <div>Vehicle: {vehicle ? `${vehicle.plate} (${vehicle.type})` : 'Missing/Invalid data'}</div>
                <div>Passengers: {run.passenger_count || 0}</div>
                {restStops.length > 0 && (
                  <div>
                    <div className="font-medium mt-2">Rest Stops:</div>
          ...
          <div className="bg-white w-full sm:w-96 sm:rounded-lg max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b p-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold">
                {editing ? 'Edit Madina Journey' : 'Add Madina Journey'}
              </h2>
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Primary Driver *</label>
                <select
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Secondary Driver (for trips &gt;4h)</label>
                <select
                  value={form.secondary_driver_id}
                  onChange={(e) => setForm({ ...form, secondary_driver_id: e.target.value })}
                  className="w-full border rounded px-3 py-2"
                >
                  <option value="">None</option>
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Rest Stops (one per line)</label>
                <textarea
                  value={form.rest_stops}
                  onChange={(e) => setForm({ ...form, rest_stops: e.target.value })}
                  placeholder="Rest Stop 1&#10;Rest Stop 2&#10;Rest Stop 3"
                  className="w-full border rounded px-3 py-2"
                  rows="4"
                ></textarea>
              </div>
